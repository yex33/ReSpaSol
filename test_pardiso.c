...
void set_ftz(void) {
  asm("stmxcsr -0x4(%rsp)\n\t"         // store CSR register on stack
      "orl     $0x8040,-0x4(%rsp)\n\t" // set bits 15 (flush-to-zero) and 7
                                       // (denormals-are-zero)
...
/* RHS and solution vectors. */
  ...
  float *b = (float *)malloc(sizeof(float) * n),
        ...
        *bs = (float *)malloc(sizeof(float) * n), res, res0;
#else
  double *b = (double *)malloc(sizeof(double) * n),
         *x = (double *)malloc(sizeof(double) * n),
         *bs = (double *)malloc(sizeof(double) * n), res, res0;
#endif
  for (int i = 0; i < n; i++) {
    b[i] = 1;
    bs[i] = 0;
  ...
  iparm[1] =
      3; /* 2 Fill-in reordering from METIS , use 3 for parallel analyze */
...
#ifdef FLOAT
  mkl_sparse_s_create_csr(&csrA, SPARSE_INDEX_BASE_ONE, n, n, ia, ia + 1, ja,
                          a);
#else
  mkl_sparse_d_create_csr(&csrA, SPARSE_INDEX_BASE_ONE, n, n, ia, ia + 1, ja,
                          a);
#endif
...
#ifdef FLOAT
  mkl_sparse_s_mv(SPARSE_OPERATION_NON_TRANSPOSE, 1.0, csrA, descrA, x, 0.0,
                  bs);
#else
  mkl_sparse_d_mv(SPARSE_OPERATION_NON_TRANSPOSE, 1.0, csrA, descrA, x, 0.0,
                  bs);
#endif
  res = 0.0;
  res0 = 0.0;
